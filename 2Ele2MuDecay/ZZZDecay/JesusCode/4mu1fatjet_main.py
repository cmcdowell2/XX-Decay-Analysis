...
if __name__ == '__main__':
    ...
    directory_bkg = ['/mnt/e/Users/Cameron/AppData/Local/Packages/CanonicalGroupLimited.UbuntuonWindows_79rhkp1fndgsc/LocalState/rootfs/home/achilles12/Physics/Background/2Ele2MuBackground/Events/run_01/tag_1_delphes_events.root']
    ...
    count_histos_bkg = {'muon': ROOT.TH1F('Bkg Muon Count', ' ; amount; events', 6, 0, 6),
                        'electron': ROOT.TH1F('Bkg Electron Count', ' ; amount; events', 6, 0, 6),
                        'fatjet': ROOT.TH1F('Bkg Fatjet Count', ' ; amount; events', 6, 0, 6)}

    # creating list output_bkg with reconstruct() output of each file in directory_bkg
    output_bkg = [reconstruct(p, z_products = {'muon': 2, 'electron': 2, 'fatjet': 1}, count_histos=count_histos_bkg) for p in directory_bkg]
    ...
    ###----------------Signal----------------###
    directory = ['/mnt/e/Users/Cameron/AppData/Local/Packages/CanonicalGroupLimited.UbuntuonWindows_79rhkp1fndgsc/LocalState/rootfs/home/achilles12/Physics/500/2E2M500/Events/run_01/tag_1_delphes_events.root']   # 500 GeV]   

    # dictionaries with histograms to count particles
    count_histos_sig = {'muon': ROOT.TH1F('Signal Muon Count', ' ; amount; events', 6, 0, 6),
                        'electron': ROOT.TH1F('Signal Electron Count', ' ; amount; events', 6, 0, 6),
                        'fatjet': ROOT.TH1F('Signal Fatjet Count', ' ; amount; events', 6, 0, 6)}

    # list of outputs from reconstruct() function for each file
    output = [reconstruct(d, z_products={'muon': 2, 'electron': 2, 'fatjet': 1}, count_histos=count_histos_sig) for d in directory]
    ...
    # Mass Histograms for the 3 bosons
    sig_boson_hists = [ROOT.TH1F('Signal Muon-Z1', ' ; mass [GeV]; events', 50, 0, 200)]
