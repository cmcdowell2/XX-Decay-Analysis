...
for mass in [500, 1000, 1500, 2000]:
    ...
    for x in ["Pt"]:
        ...
        with open("txtFiles/" + x + "Values" + str(mass) + ".txt") as f:
            signal = f.read().splitlines()
        with open("txtFiles/ZZZ" + x + "Values" + str(mass) + ".txt") as f:
        ...
        #remove_zeros(bkg_hist, signal_hist)
        ...
        while (count < 1000):
            if (math.fabs(CLs_exp0 - 0.05) < 0.000005):
                results[x + str(mass)] = scale0
                CLs[x + str(mass)] = CLs_exp0
                break
