...
void XXZRecon(const char *filepath, const char *outputFilename) {
  ...
  for (int i = 0; i < nEntries; i++) {
	...
	if (electrons.size() >= 2 && positrons.size() >= 2 && Pho_Size >= 1) {
		...
		float ptMass = findMass(photons, bosons, "pt");
		float phiMass = findMass(photons, bosons, "phi");
		float etaMass = findMass(photons, bosons, "eta");
