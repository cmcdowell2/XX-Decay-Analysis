...
float findMass(vector<TLorentzVector> list, TLorentzVector jet, string method);
...
float findMass(vector<TLorentzVector> list, TLorentzVector jet, string method) {
	...
	for (int i = 0; i < list.size(); i++) {
		...
		options.clear();
		p1 = list.at(i);
		options.push_back(p1 + jet);

		if (method == "pt") 
			deltas.push_back(fabs(p1.Pt() - jet.Pt()));
		else if (method == "eta")
			deltas.push_back(fabs(p1.Eta() - jet.Eta()));
		else if (method == "phi")
			deltas.push_back(fabs(p1.Phi() - jet.Phi()));
