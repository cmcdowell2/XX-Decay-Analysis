...
void XXZRecon(const char *filepath, const char *outputFilename) {
  ...
  vector<TLorentzVector> electrons, positrons, bosons, photons, jets;
  ...
  for (int i = 0; i < nEntries; i++) {
	...
	if (electrons.size() >= 1 && positrons.size() >= 1 && Pho_Size >= 1 && Jet_Size >= 1) {
		...
		float ptMass = findMass(photons, jets, "pt");
		float phiMass = findMass(photons, jets, "phi");
		float etaMass = findMass(photons, jets, "eta");
