...
float findMass(vector<TLorentzVector> list, vector<TLorentzVector> bosons, string method);
...
void XXZRecon(const char *filepath, const char *outputFilename) {
  ...
  vector<TLorentzVector> electrons, positrons, Ebosons, muons, antimuons, Mbosons, photons;
  ...
  for (int i = 0; i < nEntries; i++) {
	...
	if (electrons.size() >= 1 && positrons.size() >= 1 && muons.size() >= 1 && antimuons.size() >= 1 && Pho_Size >= 1) {
		...
		Ebosons = pairLeps(electrons, positrons);
		Mbosons = pairLeps(muons, antimuons);


		Ebosons = findFinal(Ebosons, 1);
		Mbosons = findFinal(Mbosons, 1);

		Z1 = Ebosons.at(0);
		Z2 = Mbosons.at(0);
		
		float ptMass = findMass(photons, Ebosons, "pt");
		float phiMass = findMass(photons, Ebosons, "phi");
		float etaMass = findMass(photons, Ebosons, "eta");
...
float findMass(vector<TLorentzVector> list, vector<TLorentzVector> bosons, string method) {
	...
	for (int i = 0; i < list.size(); i++) {
		...
		options.clear();
		p1 = list.at(i);
		for (int j = 0; j < bosons.size(); j++) {
			Z = bosons.at(j);
			options.push_back(p1 + Z);
			if (method == "pt") 
				deltas.push_back(fabs(p1.Pt() - Z.Pt()));
			else if (method == "eta")
				deltas.push_back(fabs(p1.Eta() - Z.Eta()));
			else if (method == "phi")
				deltas.push_back(fabs(p1.Phi() - Z.Phi()));
		}
		if (deltas.size() > 0) {
